...
//like and comment button for user page 
function toggleLike(button) {
  let likeCount = button.nextElementSibling; // Get like count span
  let currentLikes = parseInt(likeCount.textContent);

  if (button.classList.contains("liked")) {
      button.classList.remove("liked");
      button.innerHTML = "♡"; // Black outlined heart
      likeCount.textContent = currentLikes - 1;
  } else {
      button.classList.add("liked");
      button.innerHTML = "❤"; // Solid red heart
      likeCount.textContent = currentLikes + 1;
  }
}
...
function addComment(input) {
  ...
  let userName = "Shikha Patel"; // You can dynamically set this
  let commentList = input.nextElementSibling; // Get the <ul> comment list

  let newComment = document.createElement("li");
  newComment.innerHTML = `<strong>${userName}:</strong> ${commentText}`; // Show username
